...
def main(args):
    ...
    for i in range(args.start_idx, args.end_idx + 1):
        ...
        try:
            ...
            dist1, dist2, _, _ = chamLoss(pred_pcd_tensor, gt_pcd_tensor)
            chamfer_dist_tensor = torch.mean(dist1) + torch.mean(dist2)
            chamfer_dist_val = chamfer_dist_tensor.item()
        ...
        for radius in args.radii:
            try:
                ...
                f1_val = f1_tensor.item()
                precision_val = precision_tensor.item()
                recall_val = recall_tensor.item()
